...
def drug_recommendation_mimic3_fn(patient: Patient):
    ...
    for visit in visits:
        ...
        # 下面是新加的
        lab_inj_flag_dict = defaultdict(list)
        ...
        for key, group in groupby(labevents_sorted, key=lambda x: x[2].date()):
            ...
            lab_inj_dict[key.__format__('%Y-%m-%d')].extend(itemid_group)

            lab_inj_flag_dict[key.__format__('%Y-%m-%d')].extend(flag_group)
        ...
        for key, group in groupby(inputevents_sorted, key=lambda x: x[2].date()):
            ...
            lab_inj_dict[key.__format__('%Y-%m-%d')].extend(inj_itemid_group)
            lab_inj_flag_dict[key.__format__('%Y-%m-%d')].extend(originalamount_group)
        ...
        # 下面是新加的
        lab_inj_flag_groups = flag_groups + originalamount_groups
        ...
        lab_inj_flag_merged_list = []
        for key in sorted_keys:
            lab_inj_flag_merged_list.append(lab_inj_flag_dict[key])
        ...
        samples.append(
            {
                ...
                'lab_inj_flag_groups': lab_inj_flag_groups,
                'lab_inj_merged_list': lab_inj_merged_list,
                'lab_inj_flag_merged_list': lab_inj_flag_merged_list
            }
        )
    if len(samples) < 1:
        return []
    ...
    samples[0]["lab_inj_flag_groups"] = [samples[0]["lab_inj_flag_groups"]]
    samples[0]["lab_inj_merged_list"] = [samples[0]["lab_inj_merged_list"]]
    samples[0]["lab_inj_flag_merged_list"] = [samples[0]["lab_inj_flag_merged_list"]]
    ...
    for i in range(1, len(samples)):
        ...
        samples[i]["lab_inj_flag_groups"] = samples[i - 1]["lab_inj_flag_groups"] + [samples[i]["lab_inj_flag_groups"]]
        samples[i]["lab_inj_merged_list"] = samples[i - 1]["lab_inj_merged_list"] + [samples[i]["lab_inj_merged_list"]]
        samples[i]["lab_inj_flag_merged_list"] = samples[i - 1]["lab_inj_flag_merged_list"] + [samples[i]["lab_inj_flag_merged_list"]]
